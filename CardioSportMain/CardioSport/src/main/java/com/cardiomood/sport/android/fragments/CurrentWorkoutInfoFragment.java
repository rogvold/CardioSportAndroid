...
public class CurrentWorkoutInfoFragment extends Fragment {
    ...
    public void refresh() {
        ...
        service.setEmail(pref.getString(ConfigurationManager.USER_EMAIL_KEY, true));
        service.setPassword(pref.getString(ConfigurationManager.USER_PASSWORD_KEY, true));
            ...
            public void onResult(JsonWorkout result) {
                try {
                    if (result != null) {
                        ...
                        workoutPlannedStart.setText(new SimpleDateFormat(ConfigurationManager.getInstance().getString(ConfigurationManager.APP_DATE_FORMAT)).format(new Date(currentWorkout.getStartDate())));
